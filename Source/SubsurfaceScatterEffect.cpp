...
SubsurfaceScatterEffect::SubsurfaceScatterEffect(NiteShader* parent, char* texture) : Effect(parent)
{
	m_pGradientTexture = 0;
	m_pDepthTexture = 0;
	m_pTextureFile = texture;
	m_pSurface = 0;
	m_pFileName = "Shaders/SubsurfaceScatter.fx";
	m_pEffectName = "SUBSURFACE SCATTER";
	m_grow = 7.5f;
	//handles
	m_pWorldViewProjHandle = 0;
	m_pWorldHandle = 0;
	m_pWorldViewHandle = 0;
...
void SubsurfaceScatterEffect::Init()
{
	...
	//Create Texture
	{
		...
		HR(D3DXCreateTextureFromFile(m_pParent->GetDevice(), tempString.c_str(), &m_pGradientTexture));
	}
	...
	//Render to texture
	HR(D3DXCreateTexture(m_pParent->GetDevice(), 512, 512, 1, D3DUSAGE_RENDERTARGET, D3DFMT_R32F,
...
void SubsurfaceScatterEffect::Render(float dt, ID3DXMesh* mesh, int numMaterials)
{
	...
	HR(m_pParent->GetDevice()->EndScene());
	HR(m_pParent->GetDevice()->GetRenderTarget(0, &m_pBackBuffer));
	HR(m_pParent->GetDevice()->SetRenderTarget(0, m_pSurface));
	HR(m_pParent->GetDevice()->Clear(0, 0, D3DCLEAR_TARGET , D3DXCOLOR(0.0f, 0.7f, 0.5f, 1.0f), 1.0f, 0));
	HR(m_pParent->GetDevice()->BeginScene());
	...
	HR(m_pParent->GetDevice()->Clear(0, 0, D3DCLEAR_ZBUFFER, D3DXCOLOR(0.0f, 0.7f, 0.5f, 1.0f), 1.0f, 0));
...
void SubsurfaceScatterEffect::Update(float dt)
{
	if (m_bCompileErrors)
		return;
	...
	lightPos = m_pParent->GetLight()->GetPosition();
	...
	m_lightSpace = m_pParent->GetWorld() * temp;
	...
	HR(m_pEffect->SetFloat(m_pGrowHandle, m_grow));
		HR(m_pEffect->SetMatrix(m_pWorldViewProjHandle, &(m_lightSpace * m_lightProj)));
	HR(m_pEffect->SetMatrix(m_pWorldViewHandle, &(m_lightSpace)));
}
